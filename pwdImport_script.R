library(RCurl)
library(XML)
pollenList <- c('Zypressengewächse (Cupressaceae)', 'Erle (Alnus)', 'Hasel (Corylus)', 'Esche (Fraxinus)', 'Birke (Betula)', 'Platane (Platanus)', 'Gräser (Poaceae)', 'Roggen (Secale)', 'Nessel- und Glaskraut (Urticaceae)', 'Ölbaum (Olea)', 'Beifuß (Artemisia)', 'Ragweed (Ambrosia)', 'Pilzsporen (Alternaria)')
...
lapply(pollenList, function(x){
  ...
  belastung <- trimws(xpathSApply(pagetree, xPathStr, xmlValue)[1])
  result <- switch(belastung, 'Keine Belastung'=0, 'niedrig'=1, 'mittel'=2, 'hoch'=3, 'sehr hoch'=4)
  if(result>0){
    record <- list(timestamp = as.numeric(Sys.time()), pollType = x, value = result)
