...
pollenListCH <- c('Erle (Alnus)','Hasel (Corylus)','Esche (Fraxinus)','Birke (Betula)','Platane (Platanus)','Buche (Fagus)','Oak (Quercus)','Gräser (Poaceae)','Beifuß (Artemisia)','Ragweed (Ambrosia)')
...
url <- itemsUrl(pia_url, paste0('eu.ownyourdata.allergy.pollination', plz))
...
lapply(pollenList, function(x){
  xPathStr <- paste0("//h3[@class='polltitle' and contains(text(),'", x, "')]/..//span[@class='invisible']")
  for(i in 1:3){
    belastung <- trimws(xpathSApply(pagetree, xPathStr, xmlValue)[i])
    result <- switch(belastung, 'Keine Belastung'=0, 'niedrig'=1, 'mittel'=2, 'hoch'=3, 'sehr hoch'=4)
    if(result>0){
      if(i==1){
